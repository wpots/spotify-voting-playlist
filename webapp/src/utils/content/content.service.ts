...
import type { IBand, IVote } from '@domain/content';
...
const getCurrentBand = async (bandId: string, userId: string) => {
  ...
  const hasMembers = currentBand.members.length > 1; // always 1 -> me
  const hasPlaylists = currentBand.playlists && currentBand.playlists?.length > 0;

  let playlistRefs = hasPlaylists
    ? await SpotifyService.getPlaylistsByBulk(currentBand.playlists as string[])
    : undefined;
